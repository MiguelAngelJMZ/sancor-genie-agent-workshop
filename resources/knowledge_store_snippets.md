...
# Guion sugerido para el taller (curación en vivo)

El orden de curación es el guion pedagógico. Mostrar el **antes/después** en cada capa:

1. **Baseline.** Con solo las 5 tablas conectadas y metadatos, ejecutar las preguntas B1–B3
   (`benchmark_questions.md`). Funcionan → el poder de los buenos comentarios de UC.
2. **La trampa (B5).** Preguntar *"¿Cuántos asegurados renovaron su póliza este año?"*. Genie no
   entiende "asegurados" → **agregar el sinónimo en General Instructions (§5)** y volver a preguntar.
   Es el momento de mayor impacto del taller.
3. **SQL expressions (§1).** Preguntar por "primaje de la cartera vigente" o "siniestros fuera de
   SLA" → agregar la medida/filtro → mejora inmediata.
4. **Example queries (§3).** Ejecutar B4 (siniestralidad > 70%) sin ejemplo → agregar §3.1 → Genie
   ahora arma el ratio correctamente y generaliza.
5. **Funciones UC (§4).** Preguntar "estado de cartera del productor 5" → agregar la función →
   respuesta verificada.
6. **Pregunta WOW (§3.5).** Cerrar con el resumen ejecutivo multi-parte.

**Regla para el facilitador:** curar **una capa a la vez** y **volver a ejecutar la misma pregunta** para
que la audiencia vea el efecto. El mensaje final: *la calidad de Genie se construye por capas, y las
capas estructuradas (metadatos, expressions, funciones) rinden más que el texto libre.*

## Notas
- Ajusta `genie_workshop.sancor` si el catálogo/schema del workspace difiere.
- Después de publicar, usa el **Monitor de conversaciones** de Genie para ver qué preguntas fallan y
  seguir curando (mejora iterativa: cambiar una cosa, volver a ejecutar el benchmark, medir el impacto).
